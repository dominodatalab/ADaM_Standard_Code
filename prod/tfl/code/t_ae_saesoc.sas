...
* Sponsor              : c999
* Study                : PILOT01
* Program              : t_ae_saesoc.SAS
* Purpose              : Create table 14.5.3
* ____________________________________________________________________________
* DESCRIPTION                                                    
*                                                                   
* Input files: adsl adae
*              
* Output files: t_ae_saesoc.rtf t_ae_saesoc.sas7bdat
*               
...
* ____________________________________________________________________________
* PROGRAM HISTORY                                                         
*  18MAY2022    | Jake Tombeur   | Original version
...
/*    General options                                                        */
/*===========================================================================*/
%let dddatanam  = t_ae_saesoc;                         /* Dataset name for dddata */
%let tflid      = 14.5.3;                              /* Table ID */
%let outname    = t_ae_saesoc;                         /* Output file name */
...
/* Dataset for safety analysis set serious teaes */
data steae_as;
...
data extrow (keep = aesoc aedecod soc_pt_disp trt_:); 
    ...
    if first.aesoc then do;
        if aesoc = 'ANY' then soc_pt_disp = 'Subjects with at least one Serious TEAE';
...
title4 justify=c "Summary of Treatment-Emergent Serious Adverse events by System Organ Class and Preferred term" ;
...
proc report data = tflw.&dddatanam split = '~'
            ...
            define trt_0_npp  / display 'n (%)'
                                    style(header) = {borderbottomcolor = black borderbottomwidth = 2 width = 12%} style(column) = {leftmargin = 4%};
