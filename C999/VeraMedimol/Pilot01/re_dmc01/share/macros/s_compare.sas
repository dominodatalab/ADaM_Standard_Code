...
*           additional summaries fropm the executuon.
...
*
*  Single TFL dataset cal within QC program
*    %s_compare(base=TFL.T1401010101,comp=TFLQC.Q_T140101010101);
*
*  Batch ADaM data call via FINAL.SAS program
*    %s_compare(base=ADAM._ALL_,
*               comp=ADAMQC._ALL_,
*               comprpt =%str(&outpath\%str(&_studyid)-&__re.-&PROGTYPE.-%sysfunc(date(),yymmddn.)-COMPARE.pdf));
*
*  Batch TFL call via FINAL.SAS program
*    %s_compare(base=TFL._ALL_,
*               comp=TFLQC._ALL_,
*               tfl=Y,
*               comprpt =%str(&outpath\%str(&_studyid)-&__re.-&PROGTYPE.-%sysfunc(date(),yymmddn.)-COMPARE_TFL.pdf));
*
*
...
*                                 Fixed bug on how splitchar was being applied to dataset labels
...
%macro ___COMP;
    ...
    %** for TFL compare, force on COMPRESS option and also values for Mask **;
    %if &TFL = %str() %then %do;
      %put %str(DEB)UG: [MACRO:S_COMPARE] TFL Flag not set;
      %goto leave;
    %end;

    %else %if %upcase(&TFL)=Y %then %do;
       %let compbl=Y;
       %let IgnoreMask=1+2+4+8+16;
        ...
        %** This means any value in direct macro call takes presedence, but if globally controlled it will be actioned for TFL compares only **;
       %if %bquote(&splitchar) = %str() %then %do;
           %if %bquote(&_split) ne %str() %then %do;
              %let splitchar=%str(&_split);
           %end;
       %end;
       footnote1 "TFL COMPARE SELECTION MADE. Compare ignores dataset label/type.  Variable length/format/informat. COMPBL applied to char vars ";
    ...
    %if %quote(&splitchar) ne %str() %then %put %str(TRA)CE: [MACRO:S_COMPARE] Split character &splitchar will be suppresed from data before compare;

    %* Check if all the ID Variables are present on the input dataset;
    %macro ___idvarchk(type=);

        * Allow the application of subsets and dropping/keeping names here;
        ...
        %do _j = 1 %to %sysfunc(countw(%bquote(&idvars.),%str( )));
          ...
          %if %sysfunc(varnum(&_dsid,&_var)) = 0 %then %do;
            ...
            %put %str(ERR)OR: Variable &_var is not present on &&&type;
            %abort cancel;
          %end;
          %else %do;
            %let rc = %sysfunc(close(&_dsid));
    ...
    data _decode;
     length varc $200.;
        ...
        * Start of Extended System information codes;
        ...
        varn = 131072;  varc = 'Variable order differs (for variables in common)';     output;** POSS DROP **;
        varn = 262144;  varc = 'Variable name has different case';                     output;** POSS DROP **;
    ...
    %macro ___precomp(type=);
        ...
        * Also, apply any split character, space removal (entirely or just repeated spaces) here;
          ...
          do __jj__ = 1 to dim(_numall_);
            ...
            if ^missing("&splitchar") then do;
              * Apply modifications to the label also;
              ...
              call symputx(cats("labeln",__jj__),vlabel(_numall_(__jj__)),"L");
        ...
        proc datasets lib=work mt=data nolist nodetails;
          ...
          * Remove or retain the label, if needed for comparison;
          modify _&type.  %if %bquote(&__dsetlbl) ne %str() %then (label="&__dsetlbl");;

          * Apply the removal of split characters to labels also (if this was specified);
    ...
    * Adding title for what is being compared and any additional pre-processing;
    ...
    %if %superq(_addstr1) ne or %superq(_addstr2) ne %then %do;
    title6 "ADDITIONAL PREPROCESSING APPLIED: &_addstr1.&_sep.&_addstr2";
    %end;
    ...
    * Final proc compare section;
    ...
    %put origsysinfo  = &origsysinfo. ;
    %put _addsysinfo = &_addsysinfo.;
    ...
    %if (%bquote(&_compnoopts) ne %superq(comp) or %bquote(&_basenoopts) ne %superq(base)) 
      or &COMPRESS = Y or &COMPBL = Y or %superq(splitchar) ne 
    %then %do;
    ...
    * One record per dataset structure - more detailed one provided in _compy_result_details;
    data _compy_result;
      ...
      length PreProc $2000.;
      if "&preproc_" = "Y" then PreProc = catx(" ; ", "Yes: &_addstr1.&_sep.&_addstr2 ", "&_compprestr", "&_baseprestr");
...
%if %index(%upcase(&base),_ALL_) %then %do;
       ...
       data ___libmissmatch ___libmatch;
          merge ___libbase (in=inbase)
            ...
            if inbase ne incomp then do;
               ...
               compstatus='Issues';
               order=-1;  ** to force missmatch library first **;
        ...
        ** It will be added to later on aftre each compare             **;
       data ___libALLcomp;
          set ___libmissmatch;
       run;
