...
* Program              :  p_ODS_open.sas
* Purpose              :  Standard ods rtf open call to be used for all tables and listings
* ____________________________________________________________________________
* DESCRIPTION
* Usage                 : %p_ODS_open(TFL_ID=          - TFL ID of output
...
%macro p_ODS_open(TFL_ID = ,
                   style = evelo,
              DataSource = ,
              ListSource = %str(),
                    type =          /* Type of output: table, listing or figure, options for macro variable: T, L or F */
                  );
...
/*        call execute('TITLE01 j=l "Study Number: &studynum" j=r "Page #ByVal(&PageVar) of _#NumPages#_";');*/
        ...
        do _i=03 to 04;
            if ~missing(TITLEs{_i})    then do;
                if index(TITLEs{_i},"#byval") > 0 then call execute(cat(vname(TITLEs{_i}),   ' j=l "',trim(TITLEs{_i}),   '";'));
    ...
    *-Open ODS RTF destination (and close listing destination)                     ;
    *------------------------------------------------------------------------------;
    ODS listing close;
		*-Update path [1];
    ODS RTF 
            file        = "Z:\Evelo\EDP1867\EDP1867-101\prod\TFLs\output\&OutFileBaseName..rtf"
            style       = &style
